...
A self-hostable service for deploying static sites from GitHub. Install on a Linux machine (e.g. Mac Mini), connect GitHub, configure domains, and serve multiple sites from one instance.
...
External ingress should forward `*.mygrupo.dev` to `localhost:8080`. Grupo routes traffic by the `Host` header.

---

## Problem

For a home lab or a small fleet of static sites, you want:

- Git push → build → deploy on hardware you control
- Multiple sites on one machine, routed by hostname
- A single ingress point that forwards traffic by hostname
...
| Custom SSL termination | Handled by reverse proxy or tunnel in front of Grupo |
...
2. Configure ingress (reverse proxy or tunnel) to forward traffic to the service HTTP port (e.g. `:8080`).
...
7. Repeat for additional projects; each gets its own hostname on the same ingress.
...
Document in README; not built into the app. Grupo listens on a single port — place a reverse proxy or secure tunnel in front to terminate TLS and forward traffic.

**Requirements:**

- Forward all site hostnames (`*.mygrupo.dev`) to `http://localhost:8080`
- Preserve the original `Host` header — Grupo routes by hostname
- TLS is handled by your ingress layer, not Grupo
...
Grupo routes by `Host` internally — no per-site ingress rules needed beyond the wildcard forward.
...
- [ ] README: GitHub App creation, ingress setup, first deploy walkthrough
...
- [ ] Wildcard ingress routes both sites correctly by hostname
